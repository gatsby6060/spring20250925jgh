        ...
        const app = Vue.createApp({
            ...
            methods: {
                ...
                fnTest() {
                    ...
                    console.log("코드는 "+ self.gCode);
                    // 카테고리로 은행을 검색합니다
                    ps.categorySearch(self.gCode, self.placesSearchCB, { useMapBounds: true });
                    
                },

                fnMaRemove(){
                    let self = this;
                    self.map = new kakao.maps.Map(self.mapContainer, mapOption);
                },

                fnSearch(){
                    let self = this;
                    // self.fnMaRemove(); //마커지우기 목적
                    for(let i=0; i<self.markerList.length; i++){
                        self.markerList[i].setMap(null); //마커를 지도에서 제거 문법
                    }
                    self.makerList= []; //그것마저 마지막에 모든 기록을 지워버림 새로 잘 입력되도록?
